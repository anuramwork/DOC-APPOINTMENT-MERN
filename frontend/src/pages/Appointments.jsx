...
const Appointments = () => {
  ...
  const {doctors,getDoctorsData, currencySymbol,token, setToken,backendUrl} = useContext(AppContext)
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
          {docSlots.length && docSlots[slotIndex].map((item, index)=>{
            return(
              <p onClick={()=>{setSlotTime(item.time)}} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${!item.isAvailable ? 'bg-red-500 text-white opacity-50' : '' } ${item.time === slotTime ? 'bg-primary text-white': 'border border-gray-200' } `} key={index}>
